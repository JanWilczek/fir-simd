...
void testFirFilterTwoVectors(const std::vector<float>& signal,
    const std::vector<float>& impulseResponse,
    std::function<std::vector<float>(const std::vector<float>&, const std::vector<float>&)> filteringFunction) {
...
void testFirFilterImpulseResponses(std::function<std::vector<float>(const std::vector<float>&, const std::vector<float>&)> filteringFunction) {
    ...
    const auto expected = applyFirFilterSingle(signal.samples[0], impulseResponse.samples[0]);

    const auto avxFilteredSignal = filteringFunction(signal.samples[0], impulseResponse.samples[0]);
